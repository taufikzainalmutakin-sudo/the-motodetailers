(() => {
  ...
  function updatePrices(rows) {
    ...
    rows.forEach(row => {
      ...
      prices[size][service] = Number(row.price);
    });
  ...
  function renderPrice() {
    ...
    table.innerHTML = `
      <h3>${esc(SIZE_NAMES[active] || active)} Motorcycle</h3>
      <div class="pricegrid">
        ${SERVICE_ORDER.map(([name, key]) => `
          <div class="priceitem">
            <span>${esc(name)}</span>
            <b>${data[key] == null ? '-' : rupiah(data[key])}</b>
          </div>
        `).join('')}
      </div>
    `;
  ...
  function updateCatalog(motors, sizes) {
    ...
    catalog = motors.map(motor => ({
      brand: motor.brand,
      model: motor.model,
      size: sizeMap.get(motor.motor_size_id)
  ...
  function renderSearch() {
    ...
    const hits = catalog.filter(motor => `${motor.model} ${motor.brand}`.toLowerCase().includes(q)).slice(0, 30);

    if (!hits.length) {
      results.innerHTML = `<div class="result"><strong>Motor tidak ditemukan.</strong></div>`;
      return;
    }
    ...
    results.innerHTML = hits.map(motor => {
      ...
      return `
        <div class="result">
          <strong>${esc(motor.model)}</strong>
          <span class="pill">${esc(motor.brand)}</span>
          <span class="pill">${esc(SIZE_NAMES[motor.size])}</span>
          <div class="pricegrid">
            ${SERVICE_ORDER.map(([name, key]) => `
              <div class="priceitem">
                <span>${esc(name)}</span>
  ...
  function updateSettings(rows) {
    ...
    if (instagram) {
      const el = document.querySelector('.social-card.instagram');
      if (el) el.href = instagram;
    }

    if (tiktok) {
      const el = document.querySelector('.social-card.tiktok');
      if (el) el.href = tiktok;
    }

    if (maps) {
      const el = document.querySelector('.social-card.maps');
      if (el) el.href = maps;
    }
    ...
    if (whatsapp) {
      ...
      const social = document.querySelector('.social-card.whatsapp');
      if (social) social.href = url;

      const bookingButton = document.querySelector('.booking-button');
      if (bookingButton) bookingButton.href = url;
    }
  }
  ...
  async function sync() {
    ...
    try {
      ...
      console.log('[TMD] Customer synced');
    } catch (error) {
      console.error('[TMD] Sync error:', error);
    } finally {
      syncing = false;
    }
  }
  ...
  // Sinkronisasi pertama saat halaman customer dibuka.
  sync();

  // Real-time: perubahan harga dari dashboard langsung diambil ulang.
  ...
  // Fallback: cek perubahan maksimal 10 detik sekali jika Realtime belum aktif.
  pollTimer = setInterval(scheduleSync, 10000);
})();
